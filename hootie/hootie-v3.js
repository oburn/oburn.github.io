...
var MSG_BEGIN = '-----BEGIN HOOTIE-v3 MESSAGE-----\n';
var MSG_END = '-----END HOOTIE-v3 MESSAGE-----';

var CHUNK_SIZE = 76;
var CHUNK_SEP = "\n";

// Encrypt the msg using the passphrase
// @param msg message to be encrypted
// @param passphrase the passphrase to use
// @return the encrypted message Base64 encoded.
function encrypt_v3(msg, passphrase) {
    var hashKey = SHA256(passphrase);
    var encStr = AESEncryptCtr(msg, hashKey, 256);
    var b64Str = Base64.encode(encStr);

    var chunkedStr = '';
    var index = 0;
    while (true) {
        var remaining = Math.min(b64Str.length - index, CHUNK_SIZE);
        if (remaining == 0) {
            break;
        }
        else if (remaining == CHUNK_SIZE) {
            chunkedStr = chunkedStr + b64Str.substr(index, CHUNK_SIZE) + CHUNK_SEP;
            index = index + CHUNK_SIZE;
        }
        else {
            chunkedStr = chunkedStr + b64Str.substr(index, remaining) + CHUNK_SEP;
            break;
        }
    }
    return chunkedStr;
...
// Decrypt the msg using the passphrase
// @param msg message to be decrypted
// @param passphrase the passphrase to use
// @return the decrypted message
function decrypt_v3(msg, passphrase) {
    var encStr = Base64.decode(msg);
    var hashKey = SHA256(passphrase);
    return AESDecryptCtr(encStr, hashKey, 256);
}
